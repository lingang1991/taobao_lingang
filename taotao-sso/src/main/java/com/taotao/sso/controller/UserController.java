...
public class UserController {
	...
	public Object checkData(@PathVariable String param, @PathVariable Integer type, String callback){
		...
		if (null != result) {
			if (null != callback) {
				//MappingJacksonValue是做回掉用，目的是将callback 和 result 拼装
				//例如浏览器：http://localhost:8084/user/check/zhangsan/3?callback=param.get(data)
				//则输出的是  param.get(data)({"status":200,"msg":"OK","data":true});
